// Negative control (the mutant): identical to specs/reference.js EXCEPT that
// PUSH while LOCKED wrongly UNLOCKS instead of rejecting. The corpus must
// discriminate it — a harness that scores this clean cannot be trusted.
  ...
  component: {
    ...
    acceptors: {
      ...
      PUSH: (model) => (proposal, { reject, next, unchanged }) => {
        if (model.state === 'LOCKED') return (next.state = 'UNLOCKED', unchanged('coins'));
        next.state = 'LOCKED';
        unchanged('coins');
      },
    },
