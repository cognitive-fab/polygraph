...
//
// The authored artifact is a SAM v2 strict-profile module ({ instance, init,
// actions, getState, setState }). Every authored/repaired module must pass a
// strict validate() gate (instance({}).validate()) at the stage boundary —
// schema/shape errors are stage-blocking, never report lines. Unloadable or
...
export function validateV2Module(mod) {
  if (!isSamV2Module(mod)) {
    const hint = mod && typeof mod.next === 'function' ? ' (it exports a bare next() — the 1.x artifact, removed in 8.0.0)' : '';
  ...
  // Profile gate the library's validate() does not enforce (#4 of the SAM-v2
  // audit): a `derived: true` modelShape key is reactor-owned state — the
  // strict profile requires reactors: [], and state computed outside the
  // acceptor step cannot be model-checked faithfully, so it is stage-blocking.
  // (`internal: true` keys are legal library usage; the checker notes them as
  // structurally unexplored, and hidden-state DIVERGENCE is caught by the
  // separate-process conformance replay downstream.)
  {
    const shape = mod.instance({}).manifest()?.modelShape || null;
    const derived = shape ? Object.keys(shape).filter((k) => shape[k] && shape[k].derived === true) : [];
    if (derived.length) {
      throw new Error(`generated module declares reactor-owned (derived: true) modelShape key(s) ${derived.map((k) => `'${k}'`).join(', ')} — the strict profile requires reactors: []; model the value as acceptor-written state instead`);
    }
  }
  ...
  // guard reads undefined and rejects, while validate() stays green.
  {
    ...
    if (steps.length > 0) {
      ...
      for (const step of steps.slice(0, 200)) {
        try {
          if (JSON.stringify(adapter.transition(initState, step.action, step.data)) !== initKey) { live = true; break; }
...
/**
 * Syntax-fix transport (used for the invariants module): write
...
export async function polygen(opts) {
  ...
  // Stage-boundary loader: the module must load AND pass the strict
  // validate() gate.
  const gatedLoad = (p) => validateV2Module(loadSpec(p));
  // Author/repair transport: re-prompt the task on an unloadable rewrite.
  const obtainModule = ({ label, prompt, path }) =>
  ...
  // This is a HARD gate one line later: buildAuthorPrompt renders
  ...
  let { source: code } = await obtainModule({
    label: 'author', prompt: buildAuthorPrompt(contract, intent, { lang }), path: codePath,
  });
  ...
  //       the code is wrong, not the invariant. The repair prompt
  ...
  for (let i = 0; i <= repairMax; i++) {
    ...
    const triage = { nondeterministic: !!lastResult.nondeterministic };
    const repairPrompt = gapsAndNotes.length
      ? buildDomainGapRepairPrompt(contract, code, gapsAndNotes, { lang })
      : buildRepairPrompt(contract, code, lastResult.violations[0], { lang, triage });
  ...
  let corpusReport = await synthesizeCorpus({ contract, code, codePath, tracesDir, call, lang, loadFn: gatedLoad });
  if (corpusReport.validation.problems.length) {
    ...
    corpusReport = await synthesizeCorpus({
      contract, code, codePath, tracesDir, call, lang, loadFn: gatedLoad,
  ...
  // this is a REAL, separate-process replay through sam-tv.mjs, not a
  ...
  // expose). Each window additionally carries the lastStep()
  ...
  let replaySummary = windows.length
    ? summarizeReplay(replaySpecResults(codePath, windows), windows)
  ...
  if (replaySummary.fails > 0) {
    ...
    ({ source: code } = await obtainModule({
      ...
      prompt: buildRepairPrompt(contract, code, violation, { lang, triage }),
      path: codePath,
    }));
    ...
    lastGaps = findDataDomainRefGaps(contract, code);
    lastResult = check({ specModule: recheckModule, contract, invariants, windows: [], maxStates });
    ...
    corpusReport = await synthesizeCorpus({ contract, code, codePath, tracesDir, call, lang, loadFn: gatedLoad });
...
async function synthesizeCorpus({ contract, code, codePath, tracesDir, call, lang, loadFn = loadSpec, feedback }) {
  ...
  // Driver semantics: the module is driven through setState/actions/getState;
  // a rejected step is recorded as a no-op window (post == pre), exactly what
  // the replayer will observe.
  const step = (pre, action, data) => {
  ...
  for (const [name, steps] of Object.entries(scenarios)) {
    let state = (specModule.init(), specModule.getState());
    const lines = [];
...
function renderReport(r) {
  ...
  L.push('- artifact: **SAM v2 strict-profile module** (`{ instance, init, actions, getState, setState }`, vendored sam-lib 2.0.0-alpha; strict validate() gate at every stage boundary)\n');
  ...
  L.push('2. Wire the machine into the real handler/reducer via its exported `actions` —\n   call the intents, do not reimplement the transition logic inline.');
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  // Every flag takes a value. A flag-shaped "value" means the real one was
  // forgotten, and must be a loud usage error rather than a silent Number(true).
  ...
  console.error(`[polygen] model=${a.model}${resolved ? ` -> ${id}` : ' (verbatim)'} artifact=sam-v2`);
