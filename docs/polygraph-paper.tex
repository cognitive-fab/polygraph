...
\begin{abstract}
\noindent Coding agents now produce substantial quantities of stateful code:
order flows, payment state machines, long-running workflows. Whether such code
can be trusted is not settled by review. The defects that dominate this class
arise from event orderings that no reviewer enumerates and no test suite
constructs, and they manifest against instances already running in production,
often long after the change that introduced them. This paper describes
\tool{polygraph}, five engines delivered as plugins to a coding agent, in which
trust is established by mechanical gates rather than by inspection. The division
of responsibility is fixed and narrow: a human declares what the system observes
and what it must never do, while authoring, execution, verification, deployment
and evolution are performed by the agent and checked by deterministic tools that
require no model at inference time. The design derives from controlled
experiments on the SysMoBench benchmark, which established that the shape of the
specification contract rather than the host language governs modeling fidelity,
and which produced a state-machine library in which five recurring failure
classes are inexpressible by construction.

We report four findings. First, trace replay and invariant-guided exploration
have disjoint blind spots, and neither subsumes the other: on the etcd Raft task,
replay identified four of five divergent specifications while exploration
identified none, and exploration identified a defect that replay cannot observe
because a correct system never drives it. A specification graded by either phase
alone is credited with more than has been established. Second, because state
outlives code, the question that gates a deployment is not whether the new
version is correct but whether it is safe for the instances the previous version
left behind. \tool{polyvers} decides this by seeding an exhaustive bounded check
with the observable states the deployed fleet holds, which renders compatibility
a computable property of deployed state rather than a matter of judgment. Third,
in a pre-registered fleet study, corpus provenance determines the verdict: the
same tool, change and gates return a pass or a failure according to the origin of
the state corpus alone, since a corpus synthesized from the prior model cannot
contain the state that model deems unreachable, and that state is precisely the
one the new rules violate. Fourth, the binding limitation is the declared
observable projection rather than the checking procedure; state outside the
projection is neither checked, migrated nor audited, by construction, and the
study's most instructive result is a miss attributable to that boundary.

We situate the approach against current durable-execution practice, in which
worker versioning determines which code executes an instance but not whether
that code preserves the invariants of the state it inherits. All results are
bounded: checking is exhaustive only over declared finite domains, is relative to
human-stated invariants, and is confined to the declared projection. These
limitations are stated where they arise. A glossary defines all technical terms.
\end{abstract}
...
\emph{Conformance and exploration fail in different directions, and neither
subsumes the other.} A specification that looks right, passes syntax, explores
without finding a violation, and satisfies plausible invariants can still
describe a system that does not exist, so replay against traces captured from
the running system is indispensable: this is the origin of the toolset's
insistence that ground truth is executed code, never expectations. The original
studies recorded the stronger form of this observation, that conformance was the
only phase that discriminated. A later replication on the same external benchmark
corrected it, and the corrected claim is the one the toolset now rests on.
Conformance cannot see defensive code that a correct system never drives;
exploration cannot see anything the stated invariants omit. A specification
graded by either phase alone receives a clean bill it has not earned.
Section~\ref{sub:replication} reports the replication that forced this
correction.
...
\subsection{The replication that corrected the first premise}\label{sub:replication}

The first premise above was originally stated in its stronger form, that
conformance is the phase that discriminates. A replication on the external
benchmark's etcd Raft task, run after the toolset was built and reported in full
with the artifacts~\cite{repo}, forced a correction, and because the corrected
claim is load-bearing for the whole lifecycle we give the evidence here rather
than in a footnote.

The replication first established a calibrated baseline: a trace corpus was
captured from a real three-node cluster, and the reference specification replayed
forty-four of forty-four windows, which validates the harness, the mapper, and
the replayer end to end. Two instruments were then run over the same corpus. Eight
defects were injected into the reference specification, and five specifications
were generated one-shot from the implementation source in fresh contexts, each
forbidden to read the reference, the invariants, or any captured trace.
Table~\ref{tab:phases} reports what each instrument caught.

\begin{table}[t]
\centering
\small
\begin{tabular}{@{}p{5.6cm}p{2.6cm}p{5.2cm}@{}}
\toprule
& \textbf{replay} & \textbf{explorer + invariants} \\
\midrule
8 defects injected into the reference & 2 of 8 & 1 of 8 definitive, 6 of 8 inconclusive \\[0.35em]
5 one-shot generated specifications & 4 of 5 & 0 of 5 \\
\bottomrule
\end{tabular}
\caption{Neither phase subsumes the other. Replay misses defensive code a correct
system never drives; exploration misses anything the stated invariants omit.}
\label{tab:phases}
\end{table}

The two misses have different causes, and both are informative. Replay missed the
heartbeat clamp of the example above, because a correct cluster never drives that
branch, and the explorer caught it. The explorer missed every generated
specification's divergence, and it was given every advantage when it did so: it
was seeded not from \code{init()} but with forty-eight distinct observable states
harvested from the captured corpus, which is the fleet-seeding this paper argues
for. Four of those five specifications demonstrably disagree with real etcd, window
by window under replay, and not one of those disagreements violates any of the
three expert invariants, because all three are silent about log matching, which is
exactly where every divergence lives. This is an invariant-adequacy result, and it
is precisely what the compatibility report's \emph{invariant adequacy: NOT
MEASURED} line exists to warn about: a clean exploration verdict against an
invariant set that omits the relevant property is worth very little, and here it
is worth nothing.

Three further observations belong with the numbers. The first is that the
projection ceiling appears in the \emph{authoring} direction as well as the
checking one. The declared projection carries the log as a count with no
per-entry terms, so a specification cannot express Raft's log-matching rule at
all; the generated specifications reached for it and were penalised, while the
reference specification passes by declining to model it. On this task conformance
rewarded the specification that attempted least, which complicates any reading of
a conformance score as a pure fidelity measure. The second is that the explorer's
verdicts here were bounded rather than clean, for the reason developed in the
threats section: the reachable state space of this task is unbounded, so
exploration terminates at the cap for every specification including the
reference, and the only available verdicts are a definitive violation or an
inconclusive bounded run. The zero of five is therefore not a case of the
explorer wrongly certifying these specifications; it is the explorer never
reaching a violation, which under this toolset's own doctrine is not a pass.
The third is a correction we made to ourselves: an earlier run of this experiment
reported that the explorer caught all eight injected defects, a figure produced
by a scoring script whose pattern matched the word ``violations'' inside the
string \emph{no invariant violations reachable}. The control row is what exposed
it, which is the argument for controls stated in the small.

The scope of the replication is narrow and we do not stretch it: five
generations, one task, one model family, one-shot, which is a signature rather
than an effect size, and an explorer column that did not discriminate among these
specifications at all at the bound used.
...
bounded review gates, costing minutes on a routine change and hours on a machine
whose intent is genuinely unsettled. Everything between them becomes an agent
action guarded by a deterministic check. The next section describes the
...
\node[rotate=90, font=\scriptsize\bfseries, text=cVers!70!black, fill=white,
      inner sep=1.5pt] at ($(p2.east)!0.5!(nvd.east)+(1.2,0)$) {invariants move first};
\end{tikzpicture}
...
the only points where a person is required. The labelled arrow on the right is
the evolution loop: after a version ships and \tool{polynv} re-checks the
recorded rules, control returns to authoring (phase 2), and it returns there
first, because a change of intent precedes the change of code. That ordering is
the rule the label names.}
\label{fig:sdlc}
\end{figure}
...
\subsection{\tool{polynv}: eliciting the invariants}\label{sub:polynv}
...
bad state with a shortest path to it as a ready reproduction. The dependency runs
both ways, which is why the audit runs both: exploration surfaces only what the
invariants name, so a divergence no stated rule mentions is invisible to it and
visible to replay, as Section~\ref{sub:replication} measures. Here the third
...
refusals are the whole of the human job. Measured against the labor the agent
now absorbs it is a small share of the total.
...
needs no API key. Machine cost is negligible at this scale: across the twelve
gate runs the harness records, a single compatibility check takes a median of
0.54 seconds and at most 0.67 seconds, and the entire first tier, including
corpus construction, completes in about nine seconds. This matters only because
a gate slow enough to be bypassed is a gate that will be bypassed. The real cost is
...
be earned. The danger is measured, not hypothetical: model-generated
specifications pass a model checker while being vacuous at a reported
rate~\cite{tlaprover}, and our own replication found the complementary failure,
an exploration that reaches no violation because the invariants omit the property
at issue (Sections~\ref{sub:replication} and~\ref{sub:polynv}). A green model
check is therefore not, on its own, evidence of anything, which is why every gate
here discloses the strength of what it checked rather than only whether it
passed.
...
what the software is and what it must never do. Neither decision is
instantaneous, and on a machine whose intent is contested the second can take
hours or a working session with polynv; the claim is not that the human effort is
trivial but that it is bounded, front-loaded, and spent on the two questions no
tool can answer, instead of being spread over every line the agent produced.
...
currently required to declare. Any machine carrying an accumulating unbounded
field, such as a counter, a sequence number, or a term, has this property, and
the same replication (Section~\ref{sub:replication}) is where it surfaced.
Every compatibility guarantee is relative to the invariants a human
...
\url{https://github.com/cognitive-fab/sam-lib/tree/v2}, 2026.
...
\texttt{eval/fleet-study/}). \url{https://github.com/cognitive-fab/polygraph}, 2026.
